#!/usr/bin/env python3
"""
Working performance comparison between Wiedemann and Gaussian elimination.
"""
...
def benchmark_single_matrix(A, GF, name):
    ...
    start_time = time.time()
    try:
        kernel_bw = block_wiedemann(A, GF, m=min(3, A.shape[0]//2), n=min(3, A.shape[0]//2))
...
def main():
    ...
    for n, density, name in test_cases:
        A, GF = create_test_matrix(n, density=density)
        ge_time, sw_time, bw_time = benchmark_single_matrix(A, GF, name)
